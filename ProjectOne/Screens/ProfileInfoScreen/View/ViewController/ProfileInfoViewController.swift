...
class ProfileInfoViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        title = R.string.locale.profileTitleFillForm()
        configureTableView()
    ...
    private func configureTableView() {
        ...
        tableView.register(AdditionalInfoViewCell.self, forCellReuseIdentifier: String(describing: AdditionalInfoViewCell.self))
        tableView.register(StartButtonViewCell.self, forCellReuseIdentifier: String(describing: StartButtonViewCell.self))
...
extension ProfileInfoViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    private func createNameCell(_ tableView: UITableView, indexPath: IndexPath, type: TextType) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: String(describing: NameViewCell.self), for: indexPath) as? NameViewCell {
    ...
    private func createButtonCell(_ tableView: UITableView, indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: String(describing: StartButtonViewCell.self), for: indexPath) as? StartButtonViewCell {
            cell.delegate = self
            return cell
        }
        return UITableViewCell()
    ...
    private func createAgeCell(_ tableView: UITableView, indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: String(describing: AgeViewCell.self), for: indexPath) as? AgeViewCell {
            cell.delegate = self
            return cell
        }
        return UITableViewCell()
    ...
    private func createAdditionalInfoCell(_ tableView: UITableView, indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: String(describing: AdditionalInfoViewCell.self), for: indexPath) as? AdditionalInfoViewCell {
            cell.delegate = self
            return cell
        }
        return UITableViewCell()
...
extension ProfileInfoViewController: NameViewCellProtocol, AgeViewCellProtocol, StartButtonViewCellProtocol {
    func startButtonDidTouch() {
        ...
        presenter.setUser(name: userName, birthDate: userBirthDate, isMale: isMale ?? true, physicalState: physicalState, weight: Int(userWeight)!, height: Int(userHeight)!)
