...
class Router: RouterProtocol {
    ...
    var window: UIWindow
    init(navigationController: UINavigationController, assembly: AssemblyProtocol, dataStorage: DataStoreManagerProtocol, window: UIWindow, tabBarController: UITabBarController) {
    ...
    func initAddProductScreen() {
        ...
        guard let addProductViewController = assembly?.createAddProduct(router: self, dataStorage: dataStorage) else { return }
        navigationController.pushViewController(addProductViewController, animated: true)
    }
    
    func showInfoProfileSetting(isShowAfterOnbording: Bool) {
        if let navigationController = navigationController {
        guard let profileInfoViewController = assembly?.createProfileInfo(router: self, dataStorage: dataStorage, isShowAfterOnbording: isShowAfterOnbording) else { return }
